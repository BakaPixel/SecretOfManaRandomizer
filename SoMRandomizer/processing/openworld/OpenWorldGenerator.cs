...
namespace SoMRandomizer.processing.openworld
{
    ...
    public class OpenWorldGenerator : RomGenerator
    {
		...
		public void owSecondHacks()
		{
			addModeSpecificHack(new ElementSwapsApply());
			// randomize bosses
		...
		public void owGenerate(byte[] origRom, byte[] outRom, String seed, RandoSettings settings, RandoContext context)
		{
			// This is gross to put here but... here we are.
			var startingChar = context.workingData.get(OpenWorldCharacterSelection.STARTING_CHARACTER);
			if (startingChar != "boy")
			{
				new StartingCharacterRandomizer().add(origRom, outRom, seed, settings, context);
			}
			settings.setInt(CommonSettings.PROPERTYNAME_CURRENT_PROGRESS, 0);
