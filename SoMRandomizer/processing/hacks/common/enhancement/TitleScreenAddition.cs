...
namespace SoMRandomizer.processing.hacks.common.enhancement
{
    ...
    public class TitleScreenAddition : RandoProcessor
    {
		...
		protected void addText(List<byte> byteList, int startPos, string text, string direction = "right")
		{
			var reveresedText = text.ToCharArray();
			Array.Reverse(reveresedText);
			var toAdd = 0;
			var toWrite = text;
			switch (direction.ToLower())
			{
				...
				case "left":
					toAdd = -1;
					toWrite = new string(reveresedText);
					break;
				...
				default:
					// Default to put all characters in the same place. We could throw an error but...
					toAdd = 0;
					toWrite = text;
					break;
			...
			foreach(char s in toWrite)
			{
				byte lowerByte = (byte)char.ToLower(s);
				if (lowerByte == 0x20)
					startPos += toAdd;
				else if (lowerByte >= 0x61 && lowerByte <= 0x7A)
				{
					byteList[startPos] = lowerByte;
					startPos += toAdd;
				}
				else
					throw new ArgumentOutOfRangeException(nameof(s), $"Character '{s}' is not a valid upper/lowercase letter or space.");
			}
		}
        ...
        protected override bool process(byte[] origRom, byte[] outRom, string seed, RandoSettings settings, RandoContext context)
        {
			...
			addText(decodedSomTitlePlacements, stringPos, titleString, "left");

			// also moppleton
			stringPos = 0x1B1A;
			addText(decodedSomTitlePlacements, stringPos, "also");

			stringPos = 0x1B37;
			addText(decodedSomTitlePlacements, stringPos, "Moppleton");

			// and novapixel - one day...
			// stringPos = 0x1B5A;
			// addText(decodedSomTitlePlacements, stringPos, "and");

			// stringPos = 0x1B77;
			// addText(decodedSomTitlePlacements, stringPos, "NovaPixel");
